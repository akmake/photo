...
| אותה תמונה, **אותה עריכה**, פעם ראשונה מול שנייה באותו תהליך | 32 שנ' מול 2.3 שנ' | נמדד על תמונה אחת. **לא מייצג:** השנייה נהנתה מזיכרון פנימי של אותה עריכה בדיוק |
| הכנה מראש בכלי הפנים, ואז עריכה **אחרת** בגודל מלא | 21.4 / 25.8 / 9.9 שנ' מול 27.6 / 31.5 / 18.2 בלי הכנה | נמדד על 3 תמונות. ההכנה חוסכת רק 4–8 שנ'. רוב הזמן הוא עבודת הכלים עצמם, שתלויה בהגדרות |
| זיהוי שנעשה בגודל אחד משמש גודל אחר? | **לא.** 640→מלא: 0.5 ואז 29.4 שנ'. 1600→מלא: 4.7 ואז 22 שנ' | נמדד. בכוונה בקוד: שימוש חוזר בין גדלים נמדד בעבר כפוגע בקצוות הריטוש |
...
| ניקוי כתמים על תמונה שלא זוהו בה פנים | 18.3 שנ' בפעם הראשונה, **7.2 בחוזרת** | נמדד. **אינו בזבוז** — תיקן 25 כתמים ו-2,781 פיקסלים של צבע. רוב הזמן בפעם הראשונה הוא זיהוי |
| פתיחת קובץ JPG בגודל מלא | **0.29 שנ'** (מתוך ~16 שנ' לתמונה) | נמדד. התצוגות הקטנות **כבר** נפתחות מוקטנות. פתיחה מוקטנת חוסכת 0.15 שנ' ב-640, ו-0 ב-1600 |
...
### 1. "להבין" כל תמונה פעם אחת, ברקע, מיד בייבוא — **נמדד: רווח צנוע בלבד**
- **בפשטות:** הזיהוי של פנים, עור, שיער ורקע נעשה מראש ונשמר.
- **אצלנו (נמדד 2026-09-14):** ההנחה ש"אחרי זה כל עריכה = 2.3 שנ'" **שגויה**. הכנה מראש בכלי הפנים
  חסכה לעריכה אחרת 4–8 שנ' מתוך 18–31. שלוש סיבות: (1) הזיהוי שמור לפי גודל, ולכן הכנה בגודל מלא
  לא עוזרת לתצוגה בפאנל ולהפך; (2) בייבוא עוד אין עריכה, אז לא ידוע אילו כלים להכין; (3) רוב הזמן
  הוא עבודת הכלים עצמם, שתלויה בהגדרות ולא ניתנת להכנה מראש.
- **מסקנה:** לא הצעד הראשון. מה שבאמת יקצר את זמן הכלים הוא סעיפים 6–7 (עבודה בקטן, מסננים מהירים).
...
### 4. לפתוח תמונה בגודל שצריך — **נבדק: כמעט אין מה להרוויח**
- **בפשטות:** קובץ JPG יודע להיפתח ישר בחצי, רבע או שמינית מהגודל.
- **אצלנו (נמדד 2026-09-14):** התצוגות הקטנות **כבר** נפתחות כך (`_decode_small` במנוע). פתיחה מלאה
  עולה 0.29 שנ' מתוך ~16 שנ' לתמונה (2%). פתיחה מוקטנת חוסכת 0.15 שנ' ביעד 640, ו-0 ביעד 1600.
  הטענה המקורית כאן ("כל תמונה נפתחת מלאה") נבעה מבדיקה של פונקציית הפתיחה הכללית בלבד — שגויה.
- **אולי שווה:** רק בלולאות של מאות תמונות לאלבום (ניתוח, סינון, דמיון — עובדים ב-1280). **לא נמדד** ביעד הזה.
- **רישיון:** Pillow — HPND.

### 5. לדלג על עבודה מיותרת — **נבדק: המקרה שנמצא אינו בזבוז**
- **אצלנו (נמדד 2026-09-14):** בתמונה שלא זוהו בה פנים, ניקוי הכתמים עדיין תיקן 25 כתמים ו-2,781
  פיקסלים של צבע. דילוג היה משנה את התוצאה. מתוך 18.3 שנ' בפעם הראשונה, ~11 הם זיהוי (שנחסך בסעיף 1).
- **מסקנה:** אין כאן תיקון מהיר. אם רוצים לחסוך — רק אחרי בדיקה מה בדיוק הכלי תיקן שם, וזו החלטת מוצר.
...
> עודכן 2026-09-14 אחרי מדידה: שני הצעדים ה"קטנים" שהיו כאן ראשונים (דילוג על תמונות בלי פנים,
> פתיחה מוקטנת) נבדקו ונפסלו — ראו סעיפים 4 ו-5 בחלק 3.

> עודכן שוב באותו יום: "הבנה בייבוא" נמדדה ונותנת רק 4–8 שנ' מתוך 18–31 (סעיף 1).

1. **מסלולים מקבילים שמתאימים את עצמם למחשב** — לעבודת רקע ולמסירה. נמדד פי 2.75. דורש אישור (משנה איך המנוע עובד).
2. **מדידה פנימית של כלי הפנים** (ניקוי כתמים והחלקת עור) — שם רוב הזמן. ואז מסננים מהירים / עבודה בקטן, רק עם בדיקת התאמה.
3. **שמירת תוצאה אחרי שלבי הפנים** — עריכת צבע מיידית.
4. **כיוון מודלים** — הגדרות המתנה, ובדיקת דיוק מופחת מודל-מודל עם בדיקת איכות.
