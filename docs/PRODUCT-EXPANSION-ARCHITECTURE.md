...
2. **Workflow של צילום** — ייבוא, סשנים/רצפים, בחירה, עריכה, אלבום, מסירה.
...
זהו החור המבני הבולט ביותר כרגע. מודל הפרויקט כבר מכיר את שלב `select`, והמנוע כבר מכיר ניתוח culling, אבל ב-V2 המשתמש עובר מ-`סשנים` ישר ל-`שלח ללקוח`.
...
- עבודה לפי מקבץ קיים, אבל בלי לייצר מחדש את הסשנים
...
`סטטוס → ייבוא → סשנים → בחירה → שליחה ללקוח → עריכה → אלבום → מסירה`
