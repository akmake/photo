...
## 1.1 החלטות בעל המוצר — 14 בספטמבר 2026 (גוברות על כל סעיף אחר במסמך)

1. **ענף אחד בלבד: `main`.** אסור לפתוח ענף, לעבור ענף או ליצור worktree (ראו `CLAUDE.md` §3). כל מקום במסמך שמזכיר branch או PR נפרד — בטל. עובדים על `main` ומקמטים לעיתים קרובות.
2. **עמודי הכספת הם הבסיס של הכפולות — לעולם לא כתמונת רקע.** כל חלק דינמי: תמונות, טקסט, מסגרות, קישוטים וצבעים.
3. **צבעים לכל כפולה בנפרד, לפי ההקשר של התמונות שבה.** אין ערכת צבעים אחת לכל האלבום. התוכנה בוחרת אוטומטית אם היא מסוגלת לעשות זאת ברמה מקצועית; אחרת הצלם. הצלם תמיד יכול לשנות.
4. **גודל:** יש אלבומים בכל הצורות, אבל כרגע מתמקדים רק בגודל של הכספת — כפולה 56×21 ס"מ (עמוד 28×21). לא לחסום הוספת גדלים בעתיד.
5. **זכויות:** הכספת נוצרה על ידי מי שהתוכנה נבנית עבורו. אין בעיית רישוי מצד המוצר.
6. **קובץ מקור:** התקבל `C:\Users\yosef dahan\Downloads\הכספת.indd` (כ־16MB). זהו קובץ INDD אמיתי, אך הוא הגיע לבדו ולא כחבילת Package: אין לידו IDML, תיקיית `Links` או `Document Fonts`. בבדיקה בינארית נמצאו הפניות חיצוניות לפחות לנכסים `321A0511.JPG`, `ai-generated-fall-leaves-picture.jpg`, `cement-texture.jpg`, `gray-concrete-textured-wall.jpg`, `mixed-brown-background.jpg` ו־`portrait-young-boy-with-horse.jpg`. לכן אפשר להשתמש בו בתוך InDesign ולייצא ממנו IDML, אך אין להניח שכל הנכסים זמינים. עדיין נדרשת `File → Package` עם Copy Fonts, Copy Linked Graphics ו־Include IDML. עד שתגיע החבילה — עובדים מה־PDF ועמוד ידני אחד בלבד.
7. **`src/album/layoutEngine.ts`** מסדר תמונות בשורות עם שוליים — לא עיצוב אלבום. אין לבנות את מערכת התבניות עליו ואין להוסיף לו תבניות.
...
**לפי החלטה 1.1.3:** ה־tokens נשמרים **לכל כפולה** (`SpreadTemplateInstance.colorOverrides`), ונבחרים לפי התמונות שבכפולה. ה־theme של התבנית הוא רק ברירת המחדל שממנה מתחילים.
...
## 13. סדר העבודה (קומיטים על `main`, לא ענפים ולא PR)
...
לא לערבב schema, importer, UI וכל 145 התבניות בקומיט אחד. יהיה בלתי אפשרי לבדוק regression.
...
3. לוודא `git status -sb` — עובדים על `main` בלבד. **אסור ליצור branch.**
...
> נסגרו ב־14.09.2026 (ראו 1.1): רישוי — אין בעיה; צבעים — לכל כפולה לפי התמונות; יחס — רק גודל הכספת כרגע. התקבל קובץ INDD יחיד, אך עדיין חסרים IDML, Links ו־Document Fonts מחבילת Package. נותרו פתוחות: אילו טקסטים עריכים ואילו קבועים; personal templates — משתמש או סטודיו.
